namespace api.casa.popular.Core.Exceptions
{
    using domain.casa.popular.Exceptions;
    ...
    public class CustomExceptions : ExceptionFilterAttribute
    {
        ...
        public override void OnException(ExceptionContext context)
        {
            ...
            else if ((context.Exception is CreatePersonException))
            {
                context.HttpContext.Response.ContentType = "application/json";
                context.HttpContext.Response.StatusCode = (int)HttpStatusCode.BadRequest;
                context.Result = new JsonResult(
                    new
                    {
                        type = "CreateIdentityUserException",
                        error = new
                        {
                            message = context.Exception.InnerException?.Message ?? context.Exception.Message,
                        }
